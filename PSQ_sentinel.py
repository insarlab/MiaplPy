...
def sequential_phase_linking(CCG, ref_row, ref_col, rows, cols, method):
    ...
    for stepp in range(len(res_d)):
        ...
        if  step_0 == 0:
            phase_ref[first_line:last_line, 0:1] = phase_ref[first_line:last_line, 0] + res_d[int(stepp)]
        else:
            phase_ref[first_line:last_line, 0:1] = phase_ref[first_line:last_line, 0] + res_d[int(stepp)] - datumshift
